...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        printf("Victory%d: count = %d, expected = %d\n", victoryCards[i], testG.supplyCount[victoryCards[i]], G.supplyCount[victoryCards[i]]);
        newAssert(testG.supplyCount[victoryCards[i]] == G.supplyCount[victoryCards[i]], "Failed -- Victory count != expected");
    }
