...
int council_roomFunc(struct gameState *state, int *currentPlayer, int *handPos) {
    //+4 Cards
    int i;
    for (i = 0; i < 4; i++)
    {
...
int great_hallFunc(int *currentPlayer, struct gameState *state, int *handPos) {
    //+1 Card
    drawCard(*currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurerFunc(state, &drawntreasure, &currentPlayer, &cardDrawn, temphand, &z);
			
    case council_room:
        return council_roomFunc(state, &currentPlayer, &handPos);
...
//      return 0;
    ...
    case great_hall:
        return great_hallFunc(&currentPlayer, state, &handPos);
